		...
		<div class="row">
			<?foreach($arResult["ITEMS"] as $arItem):?>
				...
				<div class="news-list-item mb-2 col-sm" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					<div class="card">
						<!--NEWS TITLE (removed decoration + h4 -> h2)-->
						<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
							<h2 class="card-title">
								<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
									<a href="<?echo $arItem["DETAIL_PAGE_URL"]?>" style="text-decoration:none"><?echo $arItem["NAME"]?></a>
								<?else:?>
									<?echo $arItem["NAME"]?>
								<?endif;?>
							</h2>
						...
						<div class="card-body">
							<!--DATE CREATED-->
							<?if($arItem["FIELDS"]["DATE_CREATE"]):?>
								<div class="news-list-view news-list-post-params">
									<span class="news-list-icon news-list-icon-calendar"></span>
									<span class="news-list-value"><?=$arItem["FIELDS"]["DATE_CREATE"];?></span>
								</div>
							<?endif;?>

							<!--ADDED: TAG <i> for preview text-->
							<?if($arParams["DISPLAY_PREVIEW_TEXT"]!="N" && $arItem["PREVIEW_TEXT"]):?>
								<p class="card-text"><i><?echo $arItem["PREVIEW_TEXT"];?></i></p>
							<?endif;?>
							...
							<?foreach($arItem["FIELDS"] as $code=>$value):?>
								...
								<?elseif(
									...
									&& (
										...
										|| $code == "DATE_ACTIVE_TO"
										|| $code == "ACTIVE_TO"
		//cut off date created			|| $code == "DATE_CREATE"
										|| $code == "TIMESTAMP_X"
									)
								):?>
									<?
								...
								<?elseif(
									...
									<!--cut off date created from post-cycle-->
								<?elseif ($value != "" && $code != "DATE_CREATE"):?> 
									<div class="news-list-view news-list-post-params">
